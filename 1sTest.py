...
if __name__ == '__main__':
    ...
    for space in args.space_list:
        ...
        for n_hash in args.n_hashes_list:
            ...
            results = pool.starmap(run_ccm_p, zip(repeat(y_valid_ordered), repeat(valid_scores), prev_bcut, next_bcut, repeat(n_cmin_buckets), repeat(n_hash), repeat(name)))
            pool.close()
            pool.join()
            loss_results, space_actual = zip(*results)
            bigE = np.sum(loss_results)
